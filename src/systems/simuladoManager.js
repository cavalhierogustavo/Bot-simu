...
async function createSimulado(draft, channel) {
  ...
  const simulado = { id, nome: draft.name, criador: draft.creator, vagas: draft.slots, modo: draft.mode, status: 'aberto', participantes: [], equipes: Array.from({ length: teamCount }, (_, index) => ({ id: `team_${index + 1}`, nome: `Equipe ${index + 1}`, lider: null, membros: [], pedidos: [] })), rodadaAtual: 0, confrontos: [], vencedor: null, canalId: channel.id, mensagemId: null, criadoEm: new Date().toISOString() };
...
async function addPlayer(simulado, user) { if (simulado.participantes.includes(user.id)) throw new Error('Você já está participando deste simulado.'); if (simulado.participantes.length >= simulado.vagas) throw new Error('Este simulado já está lotado.'); simulado.participantes.push(user.id); await save(simulado); await store.update('jogadores', (players) => players.some((player) => player.userId === user.id) ? players.map((player) => player.userId === user.id ? { ...player, nome: user.username } : player) : [...players, { userId: user.id, nome: user.username, simuladosParticipados: [simulado.id], vitorias: 0, derrotas: 0 }]); return simulado; }
...
async function start(simulado) {
  ...
  simulado.status = 'em_andamento'; simulado.rodadaAtual = 1; await save(simulado); return simulado;
}
